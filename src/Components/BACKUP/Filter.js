...
async function fetchChildDataOfSalesOrder(filterType,response){
    ...
    for await(const data of response.data){
        await djangoInventory.get(`http://127.0.0.1:8000/${filterType}/${data.id}/customer/`)
        .then(results=>{
...
async function fetchChildDataOfProduct(filterType,response){
    for await(const data of response.data){  
        ...
        if(data.group_id !== null){            
            await djangoInventory.post(`http://127.0.0.1:8000/${filterType}/${data.id}/getProductGroupOfEachItem/`,{"group_id":data.group_id})
            .then(result=>{
...
async function fetchChildDataOfProductGroup(filterType,response){
    for await(const data of response.data){  
        console.log(data.group_id , filterType);    
            await djangoInventory.get(`http://127.0.0.1:8000/${filterType}/${data.group_id}/stockOnHand`)
            .then(result=>{
                console.log(result);
...
async function fetchProductGroup(filterType){
    ...
    .catch(error=>{          
        ...
        dispatch({
            type:'FETCH_ERROR',
            payload:'Unable to Mother fetch',
        })
    })
}
